...
## (06_genetica_cuantitativa.R es fuente por app.R antes que este
## archivo; no se usa source() aquí.)
...
## ggplot2 es una dependencia dura de este archivo (graficos).
## magick/av solo son necesarios para exportar GIF/MP4 y se cargan
## de forma opcional (guardada) para que la app no falle si no
## estan instalados; en ese caso simplemente se deshabilita esa
## funcion de exportacion en la UI.
.magick_disponible <- requireNamespace("magick", quietly = TRUE)
.av_disponible      <- requireNamespace("av", quietly = TRUE)
...
graficar_estado_hca <- function(estado, N, resist, archivo,
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x = x, y = y, colour = .data[[variable]])) +
    ggplot2::geom_point(size = point_size, alpha = alpha) +
    ggplot2::scale_colour_viridis_c(option = "plasma", name = etiqueta_leyenda) +
    ggplot2::coord_fixed() +
    ggplot2::labs(title = titulo, subtitle = subtitulo, x = "Posición X", y = "Posición Y") +
    ggplot2::theme_minimal()

  ggplot2::ggsave(archivo, plot = p, width = width, height = height,
...
animar_historial_hca <- function(historial, archivo_gif = "figuras/evolucion.gif",
  ...
  for (i in seq_along(historial)) {
    ...
    p_i <- ggplot2::ggplot(df_i, ggplot2::aes(x = x, y = y, colour = .data[[variable]])) +
      ggplot2::geom_point(size = point_size, alpha = alpha) +
      ggplot2::scale_colour_viridis_c(option = "plasma", name = etiqueta_leyenda,
    ...
    ggplot2::ggsave(archivo_i, plot = p_i, width = width, height = height,
                   units = "px", dpi = 150, bg = "white")
    archivos_frame[i] <- archivo_i
  }

  if (!is.null(archivo_gif)) {
    if (!.magick_disponible) stop("El paquete 'magick' no esta instalado; no se puede generar el GIF.")
  ...
  if (!is.null(archivo_mp4)) {
    if (!.av_disponible) stop("El paquete 'av' no esta instalado; no se puede generar el MP4.")
