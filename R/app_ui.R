  ...
  shiny::fluidPage(
    ...
    shiny::sidebarLayout(
      shiny::sidebarPanel(
        ...
        shiny::br(), shiny::br(),
        shiny::numericInput("n_reps", "Número de réplicas", value = 5, min = 2, max = 200, step = 1),
      ...
      shiny::mainPanel(
        ...
        shiny::tabsetPanel(
          id = "main_tabs",
          shiny::tabPanel("Dinámica poblacional", mod_pop_ui("pop")),
          shiny::tabPanel("Genética y fenotipo", mod_genetics_ui("gen")),
          shiny::tabPanel("Vista espacial", mod_spatial_ui("spatial")),
          shiny::tabPanel("Réplicas (rescate)", mod_replicates_ui("reps"))
        )
      )
    )
  )
