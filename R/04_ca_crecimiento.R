...
## ------------------------------------------------------------
## Aceleración opcional vía Rcpp (deshabilitada en esta app).
## Si en el futuro se agrega src/crecer_colonia.cpp con una
## función crecer_colonia_cpp(estado, g, e, N, A, ...), colocar
## el archivo .cpp junto a esta carpeta R/ (o en src/, si se
## convierte esto a un paquete) y descomentar el bloque de abajo
## para que `crecer_colonia()` use la versión compilada.
## ------------------------------------------------------------
# cpp_path <- file.path("src", "crecer_colonia.cpp")
# if (requireNamespace("Rcpp", quietly = TRUE) && file.exists(cpp_path)) {
#   tryCatch({
...
#     }
#   }, error = function(err) {
#     message("Rcpp compilation failed, using R version of crecer_colonia(): ", err$message)
#   })
# }
