...
params <- list(
  ...
  historial_dir = file.path(project_root, "example", "profile_historial")
)

profile_dir <- file.path(project_root, "example")
dir.create(profile_dir, recursive = TRUE, showWarnings = FALSE)
profile_file <- file.path(profile_dir, "profile_simular_hca.out")
