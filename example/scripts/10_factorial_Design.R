...
# ------------------------------------------------------------------
# This script is intended to be run from the project root or from the
# example/scripts directory. It locates the project root dynamically and
# writes the experiment outputs under example/ so that the example workflow
...
exp_data_dir <- file.path(project_root, "example", "data", "experiments", "factorial_core")
exp_plots_dir <- file.path(project_root, "example", "plots", "experiments", "factorial_core")
...
#
# Parameter guide:
#   A_max: amplitude of the antibiotic stress after the shock.
#   omega2: width of the stabilising selection curve; smaller values make selection tighter.
#   mutacion_sd: standard deviation of the Gaussian mutation step.
#   paso_introduccion: step at which the antibiotic treatment is introduced.
#   D, k, dt: diffusion and nutrient dynamics.
#   p_max, N_half: growth and resource-uptake parameters.
#   mort_base, mort_estres, K_mort: death and stress-dependent mortality.
#   Nc: quasi-extinction threshold used to classify a run as extinct.
base_params <- list(
  nx = 150,
  ny = 150,
  n_steps = 500,
